    ...
    if @station_to.nil?
      ...
      if params[:from_station] == 'true'
        ...
        if @station_from.nil?
          ...
          if Line.find_first_by_stations([@station_from.id, @station_to.id]).nil?
            flash[:error] = 'Brak polaczen miedzy przystankiem odjazdowym a docelowym.'
            render :action => 'new_search'
          else
            ...
            _schedules = Schedule.all(
              :conditions => ["line_id IN (?) AND station_id = ? AND arrival_at > ? AND sunday = ? AND saturday = ? AND work = ?",
                ...
                Time.now.wday != (6 and 0) ? true: false
              ],
              :order => "arrival_at",
              :limit => 10
            )
      ...
      if params[:from_my_location] == 'true'
        ...
        @stations_near.each do |station|
          ...
          _schedules = Schedule.all(
            :conditions => ["line_id IN (?) AND station_id = ? AND arrival_at > ? AND sunday = ? AND saturday = ? AND work = ?",
              lines.collect(&:id), station.id, Time.now + 3600,
              Time.now.wday == 0 ? true : false,
              Time.now.wday == 6 ? true : false,
              Time.now.wday != (6 and 0) ? true: false
            ],
