...
system('mkdir vendor/line_roads')

Line.all.each do |line|
  File.open("vendor/line_roads/ID#{line.id}##{line.number}_#{line.direction}.kml", 'w') do |f|
    f.puts '<?xml version="1.0" encoding="UTF-8"?>'
    f.puts '<kml xmlns="http://earth.google.com/kml/2.2">'
    f.puts '  <Document>'
    f.puts "    <name>#{line.number}_#{line.direction}</name>"
    f.puts '    <LookAt>'
    ...
    line.stations.each_with_index do |station,i|
      f.puts "          #{station.lng},#{station.lat},#{i}"
    end
    f.puts "        </coordinates>"
    f.puts "      </LineString>"
    f.puts "    </Placemark>"
